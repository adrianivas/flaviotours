// Comprehensive tour data for Flavio Tours
const TOURS_DATA = [
  { 
    id: 'vatican', 
...
// Ensure all tour images are local (no external fetches). Any http(s) image URLs are replaced
// with a local fallback image based on tour type.
(() => {
  const LOCAL_IMAGE_BY_TYPE = {
    vatican: 'img/vatican.svg',
    colosseum: 'img/colosseum.svg',
    city: 'img/city.svg',
    food: 'img/food.svg',
    cooking: 'img/food.svg',
    daytrip: 'img/daytrip.svg',
    night: 'img/night.svg',
    underground: 'img/underground.svg',
    default: 'img/city.svg'
  };

  const toLocalImage = (value, fallback) => {
    if (!value) return fallback;
    const str = String(value);
    return /^https?:\/\//i.test(str) ? fallback : str;
  };

  for (const tour of TOURS_DATA) {
    const fallback = LOCAL_IMAGE_BY_TYPE[tour.type] || LOCAL_IMAGE_BY_TYPE.default;
    tour.image = toLocalImage(tour.image, fallback);
    if (Array.isArray(tour.images)) {
      tour.images = tour.images.map(img => toLocalImage(img, fallback));
    }
  }
})();
...
function matchesTourFilters(tour, filters) {
  ...
  if (filters.search) {
    ...
    const haystack = [
      ...
      ...(Array.isArray(tour.included) ? tour.included : []),
      ...(Array.isArray(tour.notIncluded) ? tour.notIncluded : [])
    ]
      .filter(Boolean)
      .join(" ")
      .toLowerCase();
...
// Expose shared data/helpers for frameworks that evaluate expressions against globalThis
// (e.g., Alpine expressions may not see top-level `const` bindings).
try {
  if (typeof globalThis !== 'undefined') {
    globalThis.TOURS_DATA = TOURS_DATA;
    globalThis.getAllowedDays = getAllowedDays;
    globalThis.matchesTourFilters = matchesTourFilters;
  }
} catch (_) {
  // no-op
}
